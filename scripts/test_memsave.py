...
def run_comparison(batch_size, image_size, feature_size, num_levels, corr_channels):
    with torch.no_grad():
        img1_features_l0 = torch.rand((batch_size, feature_size, *image_size))
        img2_features_l0 = torch.rand((batch_size, feature_size, *image_size))

        attention_weights = (torch.nn.Conv3d(2, corr_channels-2, kernel_size=3, padding=1), torch.nn.Conv3d(2, corr_channels-2, kernel_size=3, padding=1))
        ...
        for level in range(num_levels):
            ...
            img2_features_lk = pyramid[level][1].contiguous()
            ...
            a_v = attention_weights[0](maxavg_u)

            # TODO: add self-adaptive correlation implementation
            # (batch, corr_channels-2,                wd/2**i,    ht, wd)
            adaptive_u = torch.zeros(batch, corr_channels-2, pyramid[level][1].shape[2], h1, w1)
            adaptive_v = torch.zeros(batch, corr_channels-2, pyramid[level][1].shape[3], h1, w1)
...
def compute_maxavg_torch(img1_features_l0 : torch.Tensor, img2_features_l0 : torch.Tensor):
...
def compute_maxavg_cuda(img1_features_l0 : torch.Tensor, img2_features_l0 : torch.Tensor):
    maxavg_u, maxavg_v = ComputeMaxAvgFunction.apply(img1_features_l0, img2_features_l0)
...
def benchmark_maxavg(batch_size, ht, wd, fdim):
    with torch.no_grad():
        img1_features_l0 = torch.rand((batch_size, ht, wd, fdim)).cuda() * 5 - 2.5
        img2_features_l0 = torch.rand((batch_size, ht, wd, fdim)).cuda() * 5 - 2.5
        print("torch:")
        print(timeit.Timer(lambda : compute_maxavg_torch(img1_features_l0, img2_features_l0)).timeit(number=100))
        ...
        for _ in range(100):
            ...
            compute_maxavg_torch(img1_features_l0, img2_features_l0)
            end_time = time.time()
        ...
        print("cuda:")
        print(timeit.Timer(lambda : compute_maxavg_cuda(img1_features_l0, img2_features_l0)).timeit(number=100))
        ...
        for _ in range(100):
            start_time = time.time()
            compute_maxavg_cuda(img1_features_l0, img2_features_l0)
            end_time = time.time()
            time_avg += end_time - start_time
        print(time_avg/100)
...
if __name__ == "__main__":
    ...
    benchmark_maxavg(5, 200,100,100)
